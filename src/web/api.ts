...
function serviceId(kind: string, value: string): string {
  return `${kind}:${Buffer.from(value).toString("base64url")}`;
}

function decodeServiceId(id: string): { kind: string; value: string } | null {
...
function buildManagedServices(plugins: PluginsConfig, proxy: ProxyConfig, registry: McpRegistry): ManagedService[] {
